...
class ImageMagick 
{
	 ...
	 * 处理图片
	 *
	 * @param string $src_file  源图
	 * @param string $dst_file  目标图片
	 * @param int $dst_width  最后宽度
	 * @param int $dst_height 最后高度
	 * @param string $suffix 缩略图后缀
	 * @param int $mode 缩略模式
	 * @param int $sharpen 是否要锐化 0,不锐化；1,锐化
	 * @return unknown
	 */
	function MagickThumb($src_file , $dst_file = '' , $dst_width , $dst_height , $suffix='_thumb' , $mode = 0 , $sharpen = 0)
	{
...
//		if (strpos($dst_file , "_") !== false)
//			$dst_file	= substr($dst_file , 0 , strrpos($dst_file, '_'))."-".$src_width."x".$src_height.substr($dst_file , strrpos($dst_file, '_'));
//		else 
//			$dst_file	= substr($dst_file , 0 , strrpos($dst_file, '.'))."-".$src_width."x".$src_height.substr($dst_file , strrpos($dst_file, '.'));
			
		if(strtolower($info['format'])=='gif') {
			if (ImageMagick::IsAnimation($src_file)) {
				ImageMagick::ResizeGif($info['mw'],$dst_file, $dst_width, $dst_height , $sharpen);return $dst_file;
			}
		}
		switch ($mode)
		{
			case 0:
				if($ratio_w > 1 && $ratio_h > 1) {
					ImageMagick::SaveImage($info['mw'], $dst_file,$info['format'] , $sharpen);
				}else{
					$ratio = $ratio_w;
			...
			case 1:
				if($ratio_w < 1 && $ratio_h < 1) { // 都缩小
					...
					ImageMagick::SaveImage($dst_mkwd, $dst_file,$info['format'] , $sharpen);
			...
			case 2:
				if($ratio_w > 1 && $ratio_h > 1) {
					...
					$dst_x = (int) abs($dst_width - $src_width) / 2 ;
					$dst_y = (int) abs($dst_height -$src_height) / 2;
					MagickCompositeImage($dst_mkwd ,$info['mw'], MW_OverCompositeOp, $dst_x, $dst_y ) ;
					ImageMagick::SaveImage($dst_mkwd, $dst_file,$info['format'] , $sharpen);
					DestroyMagickWand($info['mw']);
				}else {
					$ratio = $ratio_w > $ratio_h ? $ratio_h : $ratio_w;
	...
	function ResizeGif($src_mw , $dst_file , $dst_width, $dst_height , $sharpen = 0) 
	{
		...
		ImageMagick::SaveImage($src_mw, $dst_file, 'gif' , $sharpen);
	}
	...
	function SaveImage($mgk , $dst_file , $format , $sharpen = 0) 
	{
		MagickSetFormat($mgk, $format);
		
		//锐化
		if ($sharpen){
			MagickUnsharpMaskImage( $mgk , 1.0, 1.0, 0.2, 0 );
			MagickUnsharpMaskImage( $mgk , 1.0, 1.0, 0.2, 0 );
			MagickContrastImage( $mgk, 1);
		}
