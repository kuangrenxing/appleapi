...
//            }
            ...
            if(false !== $image) {
                ...
                $thumbHeight   =  explode(',',$this->thumbMaxHeight);
                $thumbSuffix = explode(',',$this->thumbSuffix);
                $thumbMode = explode(',',$this->thumbMode);
                $thumbSharpen = explode(',',$this->thumbSharpen);
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    $thumbMode[$i] = isset($thumbMode[$i]) ? $thumbMode[$i] : 0;
                    $thumbSharpen[$i] = isset($thumbSharpen[$i]) ? $thumbSharpen[$i] : 0;
                	$thumbname = ImageMagick::MagickThumb($filename,'',$thumbWidth[$i],$thumbHeight[$i],$thumbSuffix[$i],$thumbMode[$i],$thumbSharpen[$i]);
                }
            }
//            unlink($filename);
        }
