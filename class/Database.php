...
abstract class Database extends Config
{
	...
	public function getQueryString($table, $fields = null, $where = null, $order = null, $count = null, $offset = null, $group = null, $toTable = null)
	{
		$table	= $this->getTable($table);
		$fields	= $fields ? $fields : '*';
		$where	= $this->getWhere($where);
	...
	{
//		$qString = $this->getQueryString($table, 'COUNT(*) AS total', $where);
//
//		//处理缓存数据 memcache by zz @05.21
//		$qKey = md5($qString);
//		Globals::requireClass('Memcached');
//		$McOptions = array(
//		'servers' => array('127.0.0.1:11212'), //memcached 服务的地址、端口，可用多个数组元素表示多个 memcached 服务
//		'debug' => false,  //是否打开 debug
//		'compress_threshold' => 10240,  //超过多少字节的数据时进行压缩
//		'persistant' => false  //是否使用持久连接
//		);
//		$memcached  = new memcached($McOptions);
//
//		if(!($datas = $memcached->get($qKey))){
//			$result	= $this->select($table, 'COUNT(*) AS total', $where);
//			$rData	= $this->fetch($result);
//			$memcached->add($qKey, $rData , 60);
//		}else{
//			$rData = $datas;
//			if (count($rData) < 1){
//				unset($rData);
...
//				$rData	= $this->fetch($result);
//				
//				$memcached->replace($qKey, $rData , 60);
//			}
//		}
	...
	public function listAllWithFields($table, $fields, $where = null, $order = null, $count = null, $offset = null, $group = null)
	{
		...
		{
//			$qString = $this->getQueryString($table, $fields, $where, $order, $count, $offset, $group);
//
//			//处理缓存数据 memcache by zz @05.21
//			$qKey = md5($qString);
//			Globals::requireClass('Memcached');
...
//		$qString = $this->getQueryString($table, $fields, $where, $order, NULL, $offset);
//
//		//处理缓存数据 memcache by zz @05.21
//		$qKey = md5($qString);
//		Globals::requireClass('Memcached');
...
//
//		if(!($datas = $memcached->get($qKey))){
//			$result = $this->select($table, $fields, $where, $order, NULL, $offset);
//			$rData = $this->fetch($result);
//			$memcached->add($qKey, $rData , 60);
//		}else{
//			$rData = $datas;
